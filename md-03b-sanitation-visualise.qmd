...
```{r}
library(usethis)
use_git_config(user.name = "Eric Brändli", user.email = "ebraendli@ethz.ch")
```
...
1.  Look at the plot that you created. What do you notice about the order of the bars / order of the legend?
    -   The order of the legend is also the order of the corresponding colors in the bars
2.  What would you want to change?
    -   Put national behind rural and urban, or not show it at all
3.  Why did we remove "safely managed sanitation services" from the data set in Task 3?
    -   Because there is no problem to be adressed for the safely managed services. The remaining data shows us how the suboptimal sanitation services are distributed.

## Task 7

1.  Run the code in the code chunk below.
2.  What do you observe when you look at the code and plot?
    -   The plots show us how many countries have a percentage amount of a specific sanitation type, also sorted by residence. It shows that most countries have little lim, od and unimp sanitation, and that most countries have a high percentage of basic sanitation. It also shows that the amount of safely managed sanitation varies greatly between countries, almost uniformly across all countries, and that no generalization is possible like for the others.
